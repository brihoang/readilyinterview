      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? "Loading..." : `${docs.length} policy documents across ${Object.keys(grouped).length} categories`}
          </p>
        </div>
      </div>
      ...
      {loading ? (
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[1, 2, 3, 4].map((j) => (
                  <div key={j} className="h-20 rounded-xl border bg-white animate-pulse" />
                ))}
              </div>
            </div>
          ))}
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {search ? "Try a different search term" : "Policy documents will appear here once loaded"}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-8">
          {Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([folder, folderDocs]) => (
            <div key={folder}>
              <div className="flex items-center gap-2 mb-3">
                <FolderOpen className="h-4 w-4 text-primary" />
                <h2 className="font-semibold text-slate-700">{folderDocs[0].category}</h2>
                <Badge variant="secondary" className="text-xs">{folder}</Badge>
                <span className="text-xs text-muted-foreground">{folderDocs.length} documents</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {folderDocs.map((doc) => (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <Badge variant="outline" className="text-xs">{doc.chunkCount} sections</Badge>
                          </div>
                        </div>
                      </div>
                    </CardContent>
